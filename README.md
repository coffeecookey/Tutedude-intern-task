...
│                                                                    │
│  ┌─────────────┐   ┌────────────────┐   ┌──────────────────────┐ │
│  │   React UI   │   │ Zustand Store  │   │   PixiJS Renderer    │ │
│  │  ChatPanel   │◄──│  positions,    │──►│  GameCanvas          │ │
│  │  UserPanel   │   │  chat, rooms,  │   │  ├─ Player entities  │ │
│  │  LocationBar │   │  presence      │   │  │  (AnimatedSprites) │ │
│  │  HelpModal   │   └────────────────┘   │  ├─ MapLoader        │ │
│  └─────────────┘           ▲             │  └─ Camera (clamp)   │ │
│                             │             └──────────────────────┘ │
│                    ┌────────────────┐                              │
...
│         ┌──────────▼────────────────▼──────────┐                  │
│         │              Socket.IO                │                  │
│         │  player:move  chat:send  disconnect   │                  │
│         │  world:state  interact:start/end       │                  │
│         │  chat:history location:update          │                  │
│         │  status:batch                          │                  │
│         └───┬──────────┬────────────┬───────────┘                  │
│             │          │            │                               │
│    ┌────────▼──┐  ┌────▼───────┐  ┌▼──────────────┐              │
│    │ Game Loop │  │ Proximity  │  │ Status Manager │              │
│    │  20 Hz    │  │  Engine    │  │ AFK: 60s idle  │              │
│    │ (50ms tick│  │ Union-Find │  │ check: 10s     │              │
│    │  batched  │  │ hysteresis │  │ batch diffs    │              │
│    │ broadcast)│  │150/195px   │  └────────────────┘              │
│    └────────┬──┘  └────┬───────┘                                   │
│             │          │                                            │
│    ┌────────▼──┐  ┌────▼───────┐  ┌───────────────┐              │
...
│    │ (in-memory│  │ Socket.IO  │  │ 50 msg/room   │              │
│    │   Map)    │  │  rooms,    │  │ 100 rooms max │              │
│    └───────────┘  │ stable IDs │  │ history copy  │              │
│                   └────────────┘  └───────────────┘              │
│                                                                    │
│    ┌───────────────────────────────────────────────────────────┐  │
│    │                  Movement Handler                          │  │
│    │  rate-limit 30ms · AABB circle collision · bounds clamp   │  │
│    │  teleport detection (MAX_SPEED × 6 threshold)             │  │
│    └───────────────────────────────────────────────────────────┘  │
│                                                                    │
│    ┌──────────────────┐                                           │
│    │     MongoDB      │  ← user sessions only                     │
│    └──────────────────┘                                           │
└────────────────────────────────────────────────────────────────────┘
```
