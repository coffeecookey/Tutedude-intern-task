  ...
  return (
    <div className="fixed right-0 top-0 h-full w-72 flex flex-col"
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 flex flex-col gap-1">
        ...
        {chatMessages.map((msg, i) => (
          <div key={`${msg.timestamp}-${msg.from}-${i}`} className="text-xs" style={{ color: t.textPrimary }}>
            ...
            {msg.text}
            <span className="ml-1" style={{ color: t.textDim, fontSize: 10 }}>{fmt(msg.timestamp)}</span>
          </div>
        ))}
        <div ref={bottomRef} />
      </div>
      <div className="flex flex-col p-2 gap-1" style={{ borderTop: `1px solid ${t.border}` }}>
        {input.length > 400 && (
          <div className="text-xs text-right" style={{ color: 500 - input.length < 50 ? t.accentAlt : t.textDim }}>
            {500 - input.length}
          </div>
        )}
        <div className="flex gap-1">
          ...
          <button className="text-xs px-2 py-1 rounded"
            style={{ background: t.border, color: t.textPrimary, opacity: input.trim() ? 1 : 0.5 }}
            disabled={!input.trim()}
